...
const SectorChart = () => {
  return (
    <div className="card-dark sector-chart">
      <h6 className="section-title mb-3">Sector Performance</h6>
      ...
      <div className="chart-legend d-flex justify-content-around mt-3">
        {data.map((item) => (
          <div key={item.name} className="legend-item d-flex align-items-center gap-2">
            <span
              className="legend-dot"
              style={{ backgroundColor: barColors[item.name] }}
            ></span>
            <small className="legend-label">{item.name}</small>
            <span className="legend-value fw-medium">{`+${item.value}%`}</span>
          </div>
        ))}
      </div>
    </div>
